...
def beta_evaluator(mom_lab):                                     # beta in the lab frame
    # beta evaluator in lab frame
    res = (mom_lab[:, 3]+mom_lab[:, 7]) / (mom_lab[:, 0]+mom_lab[:, 4])      # beta = (pz_a+pz_b)/(E_a+E_b))
    return res

def random_boost(mom_lab, n_part):
    # apply a random boost to the momenta and a rotation, both spect to the z-axis
    beta = beta_evaluator(mom_lab)
    # boost the momenta by a random beta lower thsn beta cm
    rand_beta = torch.rand(mom_lab.shape[0]) - 0.5
    beta = beta * rand_beta 
    gamma = 1 / torch.sqrt(1-torch.square(beta))
    res = mom_lab.detach().clone() 
    for i in range(n_part):
        res[:, 0+i*4] = gamma*mom_lab[:, 0+i*4] - beta*gamma*mom_lab[:, 3+i*4] 
        res[:, 1+i*4] = mom_lab[:, 1+i*4] 
        res[:, 2+i*4] = mom_lab[:, 2+i*4] 
        res[:, 3+i*4] = - beta*gamma*mom_lab[:, 0+i*4] + gamma*mom_lab[:, 3+i*4] 
    # Rotate momenta around the z-axis by a random angle
    rand_angles = torch.rand(mom_lab.shape[0]) * 2 * np.pi
    for i in range(2, n_part):
        px = res[:, 1+i*4].detach().clone()
        py = res[:, 2+i*4].detach().clone()
        res[:, 1+i*4] = torch.cos(rand_angles) * px - torch.sin(rand_angles) * py
        res[:, 2+i*4] = torch.sin(rand_angles) * px + torch.cos(rand_angles) * py
    return res

def data_augmentation_boost(mom_lab, n_aug, n_part):
    # apply random_boost n_aug times to the dataset
    for i in range(n_aug):
        if i==0:
            res = torch.cat((mom_lab, random_boost(mom_lab, n_part)), axis=0)
        else:
            res = torch.cat((res, random_boost(mom_lab, n_part)), axis=0) 
    # shuffle the events
    res = res[torch.randperm(res.shape[0])]
    return res
...
# Split dataset 
train_400k, test = train_test_split(data, test_size=10**5, random_state=seed_all)
train_400k, val = train_test_split(train_400k, test_size=2*10**5, random_state=seed_all)

# generate 3 different training sets, 2 with data augnentation
train_400k, train_boost_1x4 = train_test_split(train_400k, test_size=10**5, random_state=seed_all)
train_boost_1x4 = data_augmentation_boost(train_boost_1x4, n_aug=4, n_part=n_part) 
train_boost_4x4 = data_augmentation_boost(train_400k, 4, n_part=n_part)

# remove x, y axis for initial particles
train_400k = np.delete(train_400k, [1, 2, 5, 6], axis=1)
train_boost_1x4 = np.delete(train_boost_1x4, [1, 2, 5, 6], axis=1)
train_boost_4x4 = np.delete(train_boost_4x4, [1, 2, 5, 6], axis=1)
val = np.delete(val, [1, 2, 5, 6], axis=1)
test = np.delete(test, [1, 2, 5, 6], axis=1)
...
train_boost_1x4[:, :-1] = (train_boost_1x4[:, :-1] - x_mean) / x_std
train_boost_4x4[:, :-1] = (train_boost_4x4[:, :-1] - x_mean) / x_std
val[:, :-1] = (val[:, :-1] - x_mean) / x_std
# output preprocessing (logarithm and standardization)
train_400k[:, -1] = torch.log(train_400k[:, -1])
train_boost_1x4[:, -1] = torch.log(train_boost_1x4[:, -1])
train_boost_4x4[:, -1] = torch.log(train_boost_4x4[:, -1])
val[:, -1] = torch.log(val[:, -1])
y_mean, y_std = torch.mean(train_400k[:, -1]), torch.std(train_400k[:, -1])
train_400k[:, -1] = (train_400k[:, -1] - y_mean) / y_std
train_boost_1x4[:, -1] = (train_boost_1x4[:, -1] - y_mean) / y_std
train_boost_4x4[:, -1] = (train_boost_4x4[:, -1] - y_mean) / y_std
val[:, -1] = (val[:, -1] - y_mean) / y_std 

batch_size = 256
train_400k_dataloader = DataLoader(train_400k, batch_size=batch_size, shuffle=True)
train_boost_1x4_dataloader = DataLoader(train_boost_1x4, batch_size=batch_size, shuffle=True)
train_boost_4x4_dataloader = DataLoader(train_boost_4x4, batch_size=batch_size, shuffle=True)
...
net_boost_1x4_training_size = train_boost_1x4.shape[0]
net_boost_4x4_training_size = train_boost_4x4.shape[0]

net_input_dim = train_400k.shape[1] - 1     # number of input features, -1 because last column is the output 
...
model_boost_1x4 = BNN_model.BNN_model(net_training_size=net_boost_1x4_training_size, net_inner_layers=net_inner_layers, net_input_dim=net_input_dim, net_activation_inner=net_activation_inner)
model_boost_4x4 = BNN_model.BNN_model(net_training_size=net_boost_4x4_training_size, net_inner_layers=net_inner_layers, net_input_dim=net_input_dim, net_activation_inner=net_activation_inner)
...
dir_name = BNN_model.create_directory(f"{path_output}/Results_{test_name}_DataBoost") 
...
loss_train_boost_1x4_dict = defaultdict(list)
loss_val_boost_1x4_dict = defaultdict(list)
loss_train_boost_4x4_dict = defaultdict(list)
loss_val_boost_4x4_dict = defaultdict(list)


for i_dataset in range(3):

    model = [model_400k, model_boost_1x4, model_boost_4x4][i_dataset]
    train_dataloader = [train_400k_dataloader, train_boost_1x4_dataloader, train_boost_4x4_dataloader][i_dataset]
    loss_train_dict = [loss_train_400k_dict, loss_train_boost_1x4_dict, loss_train_boost_4x4_dict][i_dataset]
    loss_val_dict = [loss_val_400k_dict, loss_val_boost_1x4_dict, loss_val_boost_4x4_dict][i_dataset]
    ...
    checkpoint_path = f"{dir_name}/state_best_mse_{['400', 'boost_1x4', 'boost_4x4'][i_dataset]}"
...
with PdfPages(f"{dir_name}/Plot_Losses_{test_name}_DataBoost.pdf") as pdf:
    for key in loss_train_400k_dict.keys():

        loss_train_400k_list = np.array(loss_train_400k_dict[key])
        loss_val_400k_list = np.array(loss_val_400k_dict[key])
        loss_train_boost_1x4_list = np.array(loss_train_boost_1x4_dict[key])
        loss_val_boost_1x4_list = np.array(loss_val_boost_1x4_dict[key])
        loss_train_boost_4x4_list = np.array(loss_train_boost_4x4_dict[key])
        loss_val_boost_4x4_list = np.array(loss_val_boost_4x4_dict[key])

        fig, ax = plt.subplots(2, 2, figsize=(8.27, 11.69)) 
        ax[0, 0].plot(np.arange(1, len(loss_train_400k_list)+1, 1), loss_train_400k_list, label=key, alpha=0.8)
        ax[0, 0].plot(np.arange(1, len(loss_val_400k_list)+1, 1), loss_val_400k_list, label=key + " val", alpha=0.8)
        ...
        ax[0, 0].set_ylabel("Loss: " + key + " 400k model")

        ax[0, 1].plot(np.arange(1, len(loss_train_boost_1x4_list)+1, 1), loss_train_boost_1x4_list, label=key, alpha=0.8)
        ax[0, 1].plot(np.arange(1, len(loss_val_boost_1x4_list)+1, 1), loss_val_boost_1x4_list, label=key + " val", alpha=0.8)
        ...
        ax[0, 1].set_ylabel("Loss: " + key + " boost_1x4 model")

        ax[1, 0].plot(np.arange(1, len(loss_train_boost_4x4_list)+1, 1), loss_train_boost_4x4_list, label=key, alpha=0.8)
        ax[1, 0].plot(np.arange(1, len(loss_val_boost_4x4_list)+1, 1), loss_val_boost_4x4_list, label=key + " val", alpha=0.8)
        ...
        ax[1, 0].set_ylabel("Loss: " + key + " boost_4x4 model")
...
pred_ori_tests = np.zeros((3, test.shape[0]))
delta_ori_tests = np.zeros((3, test.shape[0]))
sigma_model_tests = np.zeros((3, test.shape[0]))
sigma_stoch_tests = np.zeros((3, test.shape[0]))
sigma_tot_tests = np.zeros((3, test.shape[0]))
...
with PdfPages(f"{dir_name}/Plot_Test_{test_name}_DataBoost.pdf") as pdf:

    # Plot total amplitude and accuracy for Original Amplitudes 
    fig, ax = plt.subplots(2, figsize=(8.27, 11.69)) 

    ax[0].hist(np.clip(y_test_ori, bins_amp_ori[0], bins_amp_ori[-1]), bins = bins_amp_ori, histtype='step', weights=weight_plt(n_test), label='Truth', color='black')
    for i_dataset in range(3):
        ax[0].hist(np.clip(pred_ori_tests[i_dataset], bins_amp_ori[0], bins_amp_ori[-1]), bins = bins_amp_ori, histtype='step', weights=weight_plt(n_test), 
                   label=f"{['400', 'boost_1x4', 'boost_4x4'][i_dataset]} model", color=colors[i_dataset], lw=1.5, alpha=0.7)
    ...
    table_data = [["Dataset", "$|\Delta|<10^{-2}$", "$|\Delta|<10^{-2}$", "$<\sigma_{model}>$", "$<\sigma_{stoch}>$"], 
                  ["400k", f"{np.sum(np.abs(delta_ori_tests[0])<0.01)*100/len(delta_ori_tests[0]):.2f}%", f"{np.sum(np.abs(delta_ori_tests[0])<0.001)*100/len(delta_ori_tests[0]):.2f}%", 
                   f"{np.mean(sigma_model_tests[0]):.2e}", f"{np.mean(sigma_stoch_tests[0]):.2e}"],
                  ["boost_1x4", f"{np.sum(np.abs(delta_ori_tests[1])<0.01)*100/len(delta_ori_tests[1]):.2f}%", f"{np.sum(np.abs(delta_ori_tests[1])<0.001)*100/len(delta_ori_tests[1]):.2f}%", 
                   f"{np.mean(sigma_model_tests[1]):.2e}", f"{np.mean(sigma_stoch_tests[1]):.2e}"],
                  ["boost_4x4", f"{np.sum(np.abs(delta_ori_tests[2])<0.01)*100/len(delta_ori_tests[2]):.2f}%", f"{np.sum(np.abs(delta_ori_tests[2])<0.001)*100/len(delta_ori_tests[2]):.2f}%", 
